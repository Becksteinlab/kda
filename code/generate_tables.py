...
# ODE Parameters
t_max = 5e0
path = "C:/Users/nikol/phy495/kinetic-diagram-analyzer/data/LaTeX_tables"
...
def generate_probability_table(data, name, path=path):
    N = len(data)
    cols = ["Theoretical", "KDA", "SymPy", "ODE"]
    rows = ["State {}".format(i+1) for i in range(N)]
    df = pd.DataFrame(data=data, index=rows, columns=cols)
    pd.options.display.float_format = '{:.2e}'.format
    df.to_latex(path + '/{}.tex'.format(name))

def generate_error_table(data, name, path=path):
    N = len(data)
    cols = ["KDA Diff", "SymPy Diff", "ODE Diff", "KDA Error", "SymPy Error", "ODE Error"]
    rows = ["State {}".format(i+1) for i in range(N)]
    df = pd.DataFrame(data=data, index=rows, columns=cols)
    pd.options.display.float_format = '{:.2e}'.format
    df.to_latex(path + '/{}.tex'.format(name))
...
#== Make Tables: probability data ==============================================

# Stack arrays
sp3_probs = np.vstack((sp3_manual, sp3_probs)).T
sp4_probs = np.vstack((sp4_manual, sp4_probs)).T
sp4wl_probs = np.vstack((sp4wl_manual, sp4wl_probs)).T
...
generate_probability_table(sp5wl_probs, name="sp5wl_probs")
generate_probability_table(sp6_probs, name="sp6_probs")

#== Make Tables: error data ====================================================

# Stack arrays
sp3_err = np.vstack((sp3_diffs, sp3_error)).T
sp4_err = np.vstack((sp4_diffs, sp4_error)).T
sp4wl_err = np.vstack((sp4wl_diffs, sp4wl_error)).T
...
generate_error_table(sp5wl_err, name="sp5wl_error")
generate_error_table(sp6_err, name="sp6_error")
