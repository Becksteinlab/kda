...
=========================================================================
This module contains a class aimed at the analysis of biochemical kinetic
diagrams, using the methods of Hill.
...
class KDA:
    ...
    """
    def __init__(self, partials=None, directional_partials=None,
                 state_probabilities=None, state_mult_funcs=None,
                 norm_func=None, sympy_prob_funcs=None,
                 lambdify_prob_funcs=None):
        self.partials = partials
        self.directional_partials = directional_partials
        self.state_probabilities = state_probabilities
        self.state_mult_funcs = state_mult_funcs
        self.norm_func = norm_func
        self.sympy_prob_funcs = sympy_prob_funcs
        self.lambdify_prob_funcs = lambdify_prob_funcs
    ...
    def get_directional_connections(target, unique_edges):
        ...
        return functools.reduce(KDA.combine, [{target: neighbors}] + [KDA.get_directional_connections(i, unique_edges) for i in neighbors])
    ...
    def generate_partial_diagrams(G):
        ...
        combinations = list(itertools.combinations(KDA.find_unique_edges(G), N_partial_edges))
    ...
    def generate_directional_partial_diagrams(partials):
        ...
        for target in range(N_targets):
            for i in range(len(partials)):
                ...
                unique_edges = KDA.find_unique_edges(diag)      # Find unique edges of that diagram
                cons = KDA.get_directional_connections(target, unique_edges)   # Get dictionary of connections
                dir_edges = KDA.get_directional_edges(cons)                    # Get directional edges from connections
